...
import { getGroups, type Group as GroupType } from '@/services/groups';
...
import { Edit, Trash2, MoreHorizontal, PlusCircle, ArrowDownCircle, ArrowUpCircle, ArrowLeftRight as TransferIcon, ChevronDown, ArrowLeft, CopyPlus, Users } from 'lucide-react';
...
export default function GroupDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
        ...
        const [fetchedAppAccounts, fetchedAppCategories, fetchedAppTags, fetchedAppGroups] = await Promise.all([
            getAccounts(), 
            getCategories(),
            getTags(),
            getGroups()
        ...
        const targetGroup = fetchedAppGroups.find(g => g.id === groupId);
        if (!targetGroup) {
        ...
        const groupCategoryNames = targetGroup.categoryIds
          .map(catId => fetchedAppCategories.find(c => c.id === catId)?.name)
        ...
        if (fetchedAppAccounts.length > 0) {
            ...
            const groupTransactions = combinedTransactions.filter(tx => 
                tx.category && groupCategoryNames.includes(tx.category)
            );
            groupTransactions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    <div className="container mx-auto py-8 px-4 md:px-6 lg:px-8">
      <div className="flex justify-between items-center mb-6">
        <div>
            ...
            <h1 className="text-3xl font-bold flex items-center">
                <Users className="mr-2 h-6 w-6" /> <span className="ml-2">{group.name} - Transactions</span>
            </h1>
        </div>
        <DropdownMenu>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <div className="flex-grow">
          <Card>
            <CardHeader>
              <CardTitle>Transactions in Group "{group.name}" ({dateRangeLabel})</CardTitle>
              <CardDescription>All transactions belonging to categories in this group.</CardDescription>
            </CardHeader>
            <CardContent>
              {filteredTransactions.length > 0 ? (
                ...
                <div className="text-center py-10"><p className="text-muted-foreground">No transactions found for this group in the selected period.</p></div>
              )}
            </CardContent>
          </Card>
        </div>
