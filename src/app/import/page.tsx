  ...
  const handleParseAndMap = () => {
   ...
   const processAndMapData = async (confirmedMappings: ColumnMapping) => {
        ...
        for (const record of csvData) {
            ...
            if (csvType === 'opening balance') {
                ...
                const parseNameFromDescriptiveString = (text: string | undefined): string | undefined => {
                    ...
                    const match = text.match(/(?:Initial balance for |Saldo inicial para:)\s*(?:["']?([^"']+)["']?|([^:]+))$/i);
                ...
                if (accountNameForOpening && (accountNameForOpening.toLowerCase().startsWith('initial balance for') || accountNameForOpening.toLowerCase().startsWith('saldo inicial para'))) {
                    console.warn(`Could not cleanly extract account name for opening balance. Row:`, record, `Extracted name attempt: ${accountNameForOpening}`);
                    if (rawDestName && !(rawDestName.toLowerCase().startsWith('initial balance for') || rawDestName.toLowerCase().startsWith('saldo inicial para'))) {
                        accountNameForOpening = rawDestName; // One last attempt using rawDestName if it's clean
                    }
                }
                ...
                if (accountNameForOpening && recordCurrency && !isNaN(recordAmount)) {
                    ...
                    accountMap.set(normalizedName, {
                        ...
                        initialBalance: recordAmount, // Set the initial balance from this row's amount
                        category: existingDetailsInMap ? existingDetailsInMap.category : category,
                    });
                } else if (csvType === 'opening balance') {
                     console.warn(`Could not process 'opening balance' row due to missing/invalid name, currency, or amount. Row:`, record, `Parsed Amount: ${recordAmount}`, `Currency: ${recordCurrency}`, `Derived Name: ${accountNameForOpening}`);
                }
                ...
                if (sourceName && (sourceType === 'asset account' || (csvType === 'withdrawal' || csvType === 'transfer'))) {
                     accountsToConsider.push(sourceName);
                }
                if (destName && (destType === 'asset account' || (csvType === 'deposit' || csvType === 'transfer'))) {
                     accountsToConsider.push(destName);
                }


                for (const accNameToConsider of [...new Set(accountsToConsider)]) { // Process unique names
    ...
    const createOrUpdateAccountsAndGetMap = async (
        csvData: CsvRecord[],
        mappingsArg: ColumnMapping, // Renamed to avoid conflict with global columnMappings
        existingAccountsParam: Account[], 
   ...
   const handleImport = async () => {
      ...
      for (const item of recordsToImport) {
          const rowNumber = rawData.indexOf(item.originalRecord) + 2; // For user-friendly error messages
          ...
          if (item.csvTransactionType === 'transfer') {
              ...
              if (!sourceAccountId || !destAccountId) {
                   const itemIndexInDisplay = updatedDataForDisplay.findIndex(d => d.originalRecord === item.originalRecord);
                   if(itemIndexInDisplay !== -1) updatedDataForDisplay[itemIndexInDisplay] = { ...item, importStatus: 'error', errorMessage: `Row ${rowNumber}: Could not find accounts for transfer: ${item.csvRawSourceName} -> ${item.csvRawDestinationName}.` };
                  errorCount++; overallError = true; continue;
              }
              if (sourceAccountId === destAccountId) {
                  const itemIndexInDisplay = updatedDataForDisplay.findIndex(d => d.originalRecord === item.originalRecord);
              ...
              // For Firefly III withdrawals, source_name is the asset account.
              // For Firefly III deposits, destination_name is the asset account.
              ...
              if (!accountIdForTx) {
                   const itemIndexInDisplay = updatedDataForDisplay.findIndex(d => d.originalRecord === item.originalRecord);
                   if(itemIndexInDisplay !== -1) updatedDataForDisplay[itemIndexInDisplay] = { ...item, importStatus: 'error', errorMessage: `Row ${rowNumber}: Could not find account ID for "${accountNameForTx}".` };
                   errorCount++; overallError = true; continue;
              }
              ...
              if(item.csvTransactionType === 'deposit' && item.amount < 0) finalAmount = -item.amount; // Should already be positive for deposit from Firefly

              if (finalAmount === 0 && item.csvTransactionType !== 'opening balance'){
                  console.warn(`Row ${rowNumber}: Transaction amount is zero for ${item.csvTransactionType} type. CSV Amount: ${item.amount}, Final Amount: ${finalAmount}. Description: ${item.description}`);
              }
      ...
      for (const payload of transactionPayloads) {
          ...
          setImportProgress(calculateProgress(importedCount + errorCount, recordsToImport.length)); 
